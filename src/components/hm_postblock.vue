<template>
  <div
    ...
    @click="$router.push({ path: `/articleDetail/${article.id}` })"
  >
    <div class="left">
  ...
  <div
    class="single3"
    v-else-if="article.type === 1 && article.cover.length >= 3"
    @click="$router.push({ path: `/articleDetail/${article.id}` })"
  >
    <p class="content">{{ article.title }}</p>
    <div class="imgs">
      <img :src="item.url" alt v-for="item in article.cover" :key="item.id" />
    </div>
    <p class="info">
      <span>{{ article.user.nickname }}</span>
      <span>{{ article.comment_length }}跟帖</span>
